...
module HexletCode
  ...
  def self.tag_helper(name, **attrs)
    as = attrs.delete(:as) || :input
    case as
    ...
    when :select then Tag.build_label(name) + Tag.build_select(name, @model[name], **attrs)
  ...
  def self.input(name, **attrs)
    ...
    @inputs += tag_helper(name, **attrs).to_s
  end
